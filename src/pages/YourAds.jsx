                ...
                <div className="customContainer bg-white mb-5 rounded-lg shadow-md p-6">
                    ...
                    <div>
                        <Splide aria-label="services slides" options={options2} className="pb-8">
                            {userAds?.product?.map((product, index) => {
                                return (
                                    <SplideSlide key={index} className="flex justify-center items-center">
                                        <div className="bg-white border border-lightOrange rounded-lg overflow-hidden">
                                            ...
                                            <div className="px-5 my-3">
                                                ...
                                                <p className="text-sm text-gray-500">Status: <span className="text-green-500 capitalize">{product.status}</span></p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 px-2 my-3">
                                                <button className="btn-block rounded-lg bg-lightOrange hover:bg-orange duration-200 text-white font-medium text-sm py-2 col-span-2" onClick={() => document.getElementById(`interestReceivedModal${index}`).showModal()}>Interests received</button>
                                                <dialog id={`interestReceivedModal${index}`} className="modal">
                                                    <div className="modal-box">
                                                        <div className="flex justify-between items-center mb-2">
                                                            <h3 className="font-bold text-lg">Interests Received:</h3>
                                                            <button><X className="h-6 w-6" onClick={() => document.getElementById(`interestReceivedModal${index}`).close()} /></button>
                                                        </div>
                                                        {receivedInterests.filter(interest => interest.listing._id === product._id).map((interest, index) => {
                                                            return (
                                                                <div key={index} className="border border-lightOrange rounded-lg p-2 mb-2">
                                                                    <p>Name: <span className="font-semibold">{interest.sender.email}</span></p>
                                                                    <p>Phone: <span className="font-semibold">********</span></p>
                                                                    <p>Address: <span className="font-semibold">Varanasi, UP, India</span></p>
                                                                    <p>Status: <span className="font-semibold capitalize">{interest.status}</span></p>
                                                ...
                                                <button onClick={() => document.getElementById(`deactivateModal${index}`).showModal()} className="btn-block rounded-lg bg-lightOrange hover:bg-orange duration-200 text-white font-medium text-sm py-2">Deactivate</button>
                                                <dialog id={`deactivateModal${index}`} className="modal">
                                                    <div className="modal-box">
                                                        <h3 className="font-bold text-lg">Are you sure?</h3>
                                                        <p className="py-4 text-sm">If you deactivate your ad, it will be no longer visible to users.</p>
                                                        <div className="modal-action">
                                                            <form method="dialog">
                                                                <button className="btn mr-2" onClick={() => document.getElementById(`deactivateModal${index}`).close()}>Cancel</button>
                                                                <button className="btn bg-lightOrange text-white hover:bg-orange" onClick={() => document.getElementById(`deactivateModal${index}`).close()}>Confirm</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </dialog>
                    ...
                    <div>
                        <Splide aria-label="services slides" options={options2} className="pb-8">
                            {userAds?.service?.map((service, index) => {
                                return (
                                    <SplideSlide key={index} className="flex justify-center items-center">
                                        <div className="bg-white border border-lightOrange rounded-lg overflow-hidden">
                                            <img src="/images/product1.jpg" className="w-full h-48 object-cover" alt="" />
                                            <div className="px-5 my-3">
                                                ...
                                                <p className="text-sm text-gray-500">Status: <span className="text-green-500">{service.status}</span></p>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 px-2 my-3">
                                                <button className="btn-block rounded-lg bg-lightOrange hover:bg-orange duration-200 text-white font-medium text-sm py-2 col-span-2" onClick={() => document.getElementById(`interestReceivedModal${index}`).showModal()}>Interests received</button>
                                                <dialog id={`interestReceivedModal${index}`} className="modal">
                                                    <div className="modal-box">
                                                        ...
                                                        {receivedInterests.filter(interest => interest.listing._id === service._id).map((interest, index) => {
                                                            return (
                                                                <div key={index} className="border border-lightOrange rounded-lg p-2 mb-2">
                                                                    <p>{index + 1}. {interest.listing._id}. {service._id}</p>
                                                                    <p>Name: <span className="font-semibold">{interest.sender.email}</span></p>
                                                                    <p>Phone: <span className="font-semibold">********</span></p>
                                                                    <p>Address: <span className="font-semibold">Varanasi, UP, India</span></p>
                                                                    <p>Status: <span className="font-semibold capitalize">{interest.status}</span></p>
                                                ...
                                                <div className="grid grid-cols-2 gap-2 px-2 my-3">
                                                    ...
                                                    <dialog id={`interestReceivedModal${index}`} className="modal">
                                                        <div className="modal-box">
                                                            ...
                                                            {receivedInterests.filter(interest => interest.listing._id === job._id).map((interest, index) => {
                                                                return (
                                                                    <div key={index} className="border border-lightOrange rounded-lg p-2 mb-2">
                                                                        <p>Name: <span className="font-semibold">{interest.sender.email}</span></p>
                                                                        <p>Phone: <span className="font-semibold">********</span></p>
